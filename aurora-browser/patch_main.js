...
const replacement = `
        ...
        if (app.isPackaged) {
            // Priority 1: Right next to the Aurora OS.app (e.g. inside Shipping folder)
...
code = code.replace(/const { app } = require\('electron'\);\s*const modelPath = path\.join\(app\.getPath\('home'\), 'Documents', 'Aurora OS', 'stealth-engine-3b\.gguf'\);/, replacement);
